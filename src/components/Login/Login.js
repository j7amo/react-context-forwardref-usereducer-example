...
const Login = (props) => {
  ...
  // 'useReducer' hook is good when want to update some part of the state depending on some other part of the state update
    ...
    (state, action) => {
      switch (action.type) {
        case "UPDATE_ENTERED_EMAIL": {
          ...
          const formIsValid =
            updatedState.emailIsValid && updatedState.passwordIsValid;
  ...
  const validateEmailHandler = () => {
    ...
    // we are referencing 'enteredEmail' state slice which we cannot be sure about if it holds the latest value!
    // we could use another form of 'setEmailIsValid' setter BUT React would give us the latest value of
    // 'emailIsValid' in this case. And it is not what we need (we need the latest value of 'enteredEmail').
  ...
  const validatePasswordHandler = () => {
    ...
    // if our next state slice value depends on previous vaLue OF THE SAME SLICE then it's OK (but not in this case because
